...
```{r}
n_unlikely <- freq_unlikely |>
  filter(perc_unlikely > 0.75) |>
  nrow()

no_core <- freq_unlikely |>
  filter(Core == 0) |>
  nrow()

no_hab <- freq_unlikely |>
  filter(perc_unlikely == 1) |>
  nrow()
```

There are `r n_lau` LAUs proposed on the San Juan Naitonal Forest, the majority of LAUs (n=`r n_unlikely`, `r n_unlikely/n_lau*100|>round(2)`%) on the San Juan are made up of less than 25% core and likely as modeled by @squiresAnthropogenicallyProtectedNaturally2024. Eleven of the LAUs have no core habitat and two LAUs have no lynx habitat at all.
